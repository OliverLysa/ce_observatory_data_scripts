...
collected_data <- purrr::map_df(collected_sheet_names, 
  ...
  row_to_names(7) %>%
  clean_names() %>%
  mutate(quarters = case_when(str_detect(na, "Period Covered") ~ na), .before = na) %>%
  tidyr::fill(1) %>%
  filter(grepl('January to December', quarters) |
           grepl('January - December', quarters)) %>%
  mutate(source = case_when(str_detect(na, "Collected in the UK") ~ na), .before = na) %>%
  tidyr::fill(2) %>%
  # make numeric and filter out anything but 1-14 in column 1
  mutate_at(c('na'), as.numeric) %>%
  filter(between(na, 1, 14)) %>%
  select(-c(
    `na`,
    sheetname,
    Var.6,
