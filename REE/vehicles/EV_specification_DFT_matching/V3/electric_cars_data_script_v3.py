...
blacklist = ['s-a', '!', ':', '-', '_', '–', '|', '#', 'quattro', 'sportback', 'quattro auto',  '4matic', 'awd', 'x-drive', 'edition 1', 's line', 'tfsi', 'tdi', 'mhev']
...
# output file name
output_file_name = 'output_v3.csv'
...
def create_tricky_model(x):
    result = x['tricky_model'].split()[0] + ''.join([element for element in x['tricky_model'].split()[1:] if element.isnumeric() or element[0] in ['e', 'i']])
    return result
...
def main():
    ...
    try:
        ...
        cars_data_df['cars_key'] = cars_data_df['Brand, model, trim, price_Model year'].astype(str).str.lower() + ' ' + cars_data_df[
            'Brand, model, trim, price_Brand'].astype(str).str.lower() + ' ' + cars_data_df['tricky_model'].astype(str).str.lower()
